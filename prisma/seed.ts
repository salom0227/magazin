...
async function main() {
  ...
  // Create/Update Categories (idempotent: safe to run on every deploy)
  const categoryData = [
    {
      name: 'Smartfonlar',
      slug: 'smartphones',
  ...
  // Create/Update Currencies (idempotent)
  const currencyData = [
    { code: 'USD', symbol: '$', rate: 12700, isActive: true },
    { code: 'EUR', symbol: '€', rate: 13800, isActive: true },
    { code: 'RUB', symbol: '₽', rate: 140, isActive: true },
    { code: 'CNY', symbol: '¥', rate: 1750, isActive: true },
  ];
  ...
  // Only seed demo products if none exist yet (products have no natural unique key)
  ...
  if (existingProductCount === 0) {
    ...
    console.log(`ℹ️  Skipping demo products (${existingProductCount} product(s) already exist)`);
  }
